...
def test_de_dup_and_sort_student():
    assert ["a","b","c","d"] == de_dup_and_sort(["a","b","c","d"])
    assert ["a", "b", "c", "d"] == de_dup_and_sort(["a","a", "b","b", "c", "d"])

    import string
    assert list(string.ascii_lowercase) == de_dup_and_sort(list(string.ascii_lowercase))
