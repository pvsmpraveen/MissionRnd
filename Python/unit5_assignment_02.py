...
def generator_zip(seq1, seq2, *more_seqs):
    if more_seqs==():
       ...
       t=min(len(seq1),len(seq2),min(map(len,more_seqs)))
       for x in range(t):
...
def test_generator_zip_student():
    gen = generator_zip(range(5), range(3), range(4), range(5),range(6))
    check_generator(gen, 3, 5)

    gen = generator_zip(range(5), range(3),range(9),[0,1,2,3,4])
